        ...
        <div class="top_right_country_more_main">
            ...
            <div class="top_right_currency_more_main_list" :class="{'close':curIsClose}" id='currencyList'>
                <div class="top_right_currency_more_main_list_main">
                    <ul>
                        <li v-for="(cur , j) in currencyResult" :key="j" @click="changeLanguage(cur)">
            ...
            <a href="javascript:;" class="top_right_country_more_save" @click="saveCLCinformation" :class='{"saving":isSaving}' id="save-CLC">Save</a>
            <!-- <div id="loader_order_alone" class="loader_order"><svg class="circular" viewBox="25 25 50 50"><circle class="path" cx="50" cy="50" r="20" fill="none" stroke-width="4" stroke-miterlimit="10"></circle></svg></div> -->
        </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeLanguage(cur) {
      this.$store.dispatch("changeDefaultLanguage", cur).then(() => {
        this.curIsClose = true;
    ...
    saveCLCinformation() {
        this.isSaving = true;
        document.getElementById("save-CLC").innerHTML =
          '<div id="loader_order_alone" class="loader_order"><svg class="circular" viewBox="25 25 50 50"><circle class="path" cx="50" cy="50" r="20" fill="none" stroke-width="4" stroke-miterlimit="10"></circle></svg></div>';
      this.$store.dispatch("saveCLC").then(() => {
    ...
    mouseOverDoc() {
      document.onmouseover = e => {
        ...
        if (target.closest(".top_main_right_help") != null) {
          document.getElementById("country-currency-choose").style.display =
            "none";
        }
    ...
    readCLCinformation(){
        this.$store.dispatch('readCLC');
    }
    // ...mapMutations(['changeDefaultCountry'])
  },
  mounted: function() {
    this.mouseOverDoc();
    this.readCLCinformation();
  }
};
</script>
...
.top_right_currency_more_main_list_main {
  width: 100%;
...
.top_right_country_more_save.saving {
